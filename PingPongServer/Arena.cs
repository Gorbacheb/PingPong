...
namespace PingPongServer
{
    ...
    public class Arena
    {
        ...
        private DateTime _lastGameState;
            ...
            while (running)
            {
                ...
                switch (State.Value)
                {
                    ...
                    case ArenaState.InGame:
                        ...
                        if (haveMsg)
                        {
                            //PlayerInfo tplayer = message.Sender.Equals(LeftPlayer.Endpoint) ? LeftPlayer : RightPlayer;
                            //Console.WriteLine(tplayer.Paddle.Side.ToString() + " " + tplayer.LastPacketReceivedTime.ToString());
                ...
                if (haveMsg && (message.Packet.Type == PacketType.Bye))
                {
                    ...
                    PlayerInfo player = message.Sender.Equals(LeftPlayer.Endpoint) ? LeftPlayer : RightPlayer;
                    running = false;
                    Console.WriteLine("[{0:000}] Quit detected from {1} at {2}", Id, player.Paddle.Side, _gameTimer.Elapsed);
        ...
        {
            // Top/Bottom
            int ballY = _ball.Position.Y;
            if ((ballY <= 0) || (ballY + GameGeometry.BallSize.Y * 2 + 22 >= GameGeometry.PlayArea.Y))
            {
                _ball.Speed.Y *= -1;
            }

            // Ball left and right (the goals!)
            int ballX = _ball.Position.X;
            if (ballX + GameGeometry.BallSize.X <= 0)
            {
                // Right player scores! (reset ball)
                RightPlayer.Paddle.Score += 1;
                Console.WriteLine("[{0:000}] Right Player scored ({1} -- {2}) at {3}", Id, LeftPlayer.Paddle.Score, RightPlayer.Paddle.Score, _gameTimer.Elapsed);
                _ball.Initialize();
            }
            else if (ballX + GameGeometry.BallSize.X >= GameGeometry.PlayArea.X)
            {
                // Left palyer scores! (reset ball)
                LeftPlayer.Paddle.Score += 1;
                Console.WriteLine("[{0:000}] Left Player scored ({1} -- {2}) at {3}", Id, LeftPlayer.Paddle.Score, RightPlayer.Paddle.Score, _gameTimer.Elapsed);
                _ball.Initialize();
            }
        ...
        private void _processBallHitWithPaddle(PaddleCollision collision)
        {
            ...
            if (collision == PaddleCollision.WithFront || collision == PaddleCollision.WithTop)
            {
                _ball.RandomizeSpeed();
            }
